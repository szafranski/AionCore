...
- ⬜ D5a/b/c 三个 prompt builder 合并；快照测试绿；模板文本来自 AionUi 源码（非大模型生成）
...
**产出物清单**：
- ⬜ D7 `TeamSession::stdio_spec / on_agent_finish / compute_wake_input` + `send_message` / `send_message_to_agent` 接 wake 合并 + 6 条单元/集成测试绿（其中含 mock send_message 被调）
- ⬜ D8 `TeammateStatus::Pending` + 首次 wake 区分合并 + 3 条单元测试绿
- ⬜ D9 `TeamSessionService::ensure_session` kill+rebuild 闭环合并 + MockWorkerTaskManager 集成测试绿（验证 `kill` 和 `get_or_build_task` 各被调 N 次、extra 被正确写回）
...
**时点**：M4 后 4 天（W4-D25a/b/c 底座 1.8 天 → 其他并行 2 天）。**可与 M5 并行**。

**产出物清单**（17 个子模块）：
- ⬜ W4-D25a `AgentStreamChunk` enum 合并 + 1 条 serde 测试绿
- ⬜ W4-D25b `subscribe_stream()` trait 方法合并 + 1 条 trait 对象测试绿
- ⬜ W4-D25c `AcpAgentManager` broadcast 注入合并 + 3 条广播测试绿
- ⬜ W4-D18a `active_wakes` 重入锁合并 + 2 条并发测试绿
- ⬜ W4-D18b `wake_timeouts` 60s 看门狗合并 + 3 条测试绿
...
- ⬜ W4-D20b 非 leader crash 流程合并 + 3 条测试绿
...
- ⬜ W4-D24b server notify + wait graceful 合并 + 3 条测试绿
...
- ⬜ W5-D26b `handle_aion_create_team` handler 合并 + 3 条测试绿
...
- ⬜ W5-D29a `SpawnAgentRequest` + 校验层合并 + 3 条测试绿
- ⬜ W5-D29b `add_agent` 扩展合并 + 2 条测试绿
- ⬜ W5-D29c 写 extra + kill + rebuild 合并 + 2 条测试绿
- ⬜ W5-D29d 欢迎消息 + wake + 事件合并 + 2 条测试绿
- ⬜ W5-D30a `shutdown_approved` 拦截合并 + 2 条测试绿
- ⬜ W5-D30b `shutdown_rejected` 拦截合并 + 2 条测试绿
- ⬜ W5-D30c `shutdown_agent` 目标 role 校验合并 + 1 条测试绿
- ⬜ W5-D30d `remove_agent` 真 kill + 清 state 合并 + 3 条测试绿
- ⬜ W5-D31a `TeamMcpPhase` + payload 类型合并 + 2 条 serde 测试绿
- ⬜ W5-D31b 10 phase 广播点合并 + 2 条集成测试绿
...
| 里程碑 | W1（9） | W2（5） | W3（16） | W4（17） | W5（19） |
|:---:|---|---|---|---|---|
| M0 (ack) | D1–D6 all ack | D7–D11 ack | — | — | — |
| M1 (merge) | D1–D6 all ✅ | — | — | — | — |
| M2 (merge) | — | D7–D11 ✅（D11 骨架） | — | — | — |
| M3 (smoke) | — | D11 smoke ✅ | — | — | — |
| M4 (W2 收尾) | post-fix | post-fix | ack（16 人） | ack（17 人） | — |
| M5 (W3 merge) | — | — | W3-D12a..D17b ✅ | — | — |
| M6 (W4 merge) | — | — | — | W4-D18a..D25c ✅ + 可靠性 smoke | — |
| M7 (W5 merge) | — | — | — | — | W5-D26a..D31c ✅ + 全量 e2e |

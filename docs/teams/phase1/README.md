...
**仅剩 4 个超 200 行例外模块**（Wave 1/2 历史遗留，leader 已批）：
- `D5b-1`（188 行 AionUi 原文 .txt + 48 行 Rust `include_str!`）——模板文本是原料不是逻辑
- `D5c`（~150 行 teammate prompt 模板 + builder）——模板原料 + 单一 builder
- `D7`（~280 行 TeamSession 新方法 + send 路径接 wake）——同文件同语义闭环（compute/consume），拆开二人同文件 merge 冲突
- `D9`（~200 行 ensure_session）——5 步原子启动流水线，中间失败需回滚整个 session，不可拆

Wave 3/4/5 共 **47 个新模块全部 ≤ 120 行**；超 100 行的模块（W3-D15b / W3-D16b / W4-D18b / W4-D20b / W4-D22 / W4-D24b / W5-D26b / W5-D29b）都在 modules.md 对应条目给出"不能再拆理由"。
...
- D4  两个 MCP 工具 descriptor + 最小 handler（`team_list_models` / `team_describe_assistant`）
...
### Wave 2（最小闭环业务串接，5 人）
> **开工准则**：依赖 Wave 1 全部完成；关键路径 D7 → D9 → D11。
- D7  `TeamSession` 新方法 + `send_message` / `send_message_to_agent` 接 wake
...
### Wave 4（鲁棒性与可靠性，17 人，W4-D25a/b/c 先做底座 → 其余并行）
> **开工准则**：依赖 Wave 2；所有订阅型模块必须先让 D25a/b/c 完成 chunk 订阅公共底座。
> **拆分原则**：同 Wave 3。
> **目标**：把 AionUi §8 列的 17 条硬约束里 phase1 Wave 2 跳过的 8 条全部落地。

- W4-D25a `AgentStreamChunk` enum / W4-D25b `subscribe_stream()` trait / W4-D25c `AcpAgentManager` broadcast 注入 + 全 chunk emit — [backend-audit P1#53](./backend-audit.md#35-交叉审阅补漏二轮对照-aionui-audit-7-8-后新发现)
- W4-D18a `active_wakes` 重入锁 / W4-D18b `wake_timeouts` 60s 看门狗 / W4-D18c session 接入 wake lock — [aionui-audit §8 #1/#2](./aionui-audit.md#8-源码中发现的硬约束agent-行为易坏点)
- W4-D19a `finalized_turns` 存储 + API / W4-D19b session 接入 dedup — [aionui-audit §8 #3](./aionui-audit.md#8-源码中发现的硬约束agent-行为易坏点)
- W4-D20a `detect_crash` 纯函数 / W4-D20b 非 leader crash 流程 / W4-D20c leader crash 分支 — [backend-audit P1#17](./backend-audit.md#32-p1能跑但体验差--关键硬约束)
...
- W4-D24a `McpReadyNotification` 协议类型 / W4-D24b server notify + wait graceful / W4-D24c bridge 发 mcp_ready — [backend-audit P1#34](./backend-audit.md#32-p1能跑但体验差--关键硬约束)
...
> **拆分原则**：同 Wave 3。Wave 5 是 phase1 最易出"超模块"的地方，已全部拆开，每模块只做一件事。
> **目标**：让"单聊→建团→真 spawn→真 kill"四条完整链路全部闭环，等价 AionUi 参考实现。

- W5-D26a `GuideMcpServer` 启停 / W5-D26b `handle_aion_create_team` / W5-D26c `handle_aion_list_models` / W5-D26d 建团后 3 个 WS 事件 — [backend-audit P1#28/#33](./backend-audit.md#32-p1能跑但体验差--关键硬约束)
...
- W5-D29a `SpawnAgentRequest` + 校验层 / W5-D29b `add_agent` 扩展 / W5-D29c 写 extra + kill + rebuild / W5-D29d 欢迎消息 + wake + spawn 事件 — [backend-audit P1#22](./backend-audit.md#32-p1能跑但体验差--关键硬约束)
- W5-D30a `shutdown_approved` 拦截 / W5-D30b `shutdown_rejected` 拦截 / W5-D30c `shutdown_agent` 目标 role 校验 / W5-D30d `remove_agent` 真 kill + 清内部 state — [backend-audit P1#21/#23/P0#20](./backend-audit.md#32-p1能跑但体验差--关键硬约束)
- W5-D31a `TeamMcpPhase` enum + payload 类型 / W5-D31b 10 phase 广播点 / W5-D31c `teammate_message` 左气泡 emit — [backend-audit P2#42 / P1#27](./backend-audit.md)
...
| Wave | 目标 | 并行人数 | 关键路径 | 总人天（单人累计） |
|------|------|:-:|:-:|:-:|
| Wave 1（原料） | 最小闭环纯净模块 | 9 人并行 | 1.5 天 | 6.6 人天 |
| Wave 2（串接） | 最小闭环业务 | 最多 3 人并行 | 7 天 | 9.5 人天 |
| Wave 3（规范化） | 多用户/协议硬上限 | 16 人并行（内部 D13 链 3 串） | 2 天 | 7.5 人天 |
| Wave 4（鲁棒性） | scheduler 可靠性 | W4-D25a/b/c 串行底座 → 14 人并行 | 4 天 | 11.7 人天 |
| Wave 5（闭环补全） | Guide MCP / spawn / shutdown | 19 人；D29 链与 D26 链可并行 | 6 天 | 12.2 人天 |
| **合计** | — | **66 人模块** | **~21 日历日** | **~46 人天** |

**建议工期**：5–6 周（Wave 1 0.5 周 + Wave 2 1 周 + Wave 3 0.5 周 + Wave 4 1 周 + Wave 5 1.5 周 + buffer 0.5 周）。
